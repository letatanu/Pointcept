...
model = dict(
    ...
    backbone=dict(
        ...
        # was 6 (color + normal), now only normals as features
        in_channels=3,
...
data = dict(
    ...
    train=dict(
        ...
        transform=[
            ...
            # remove chromatic ops since we drop color
            ...
            # dict(type="NormalizeColor"),  # no color now
            dict(type="ToTensor"),
            dict(
                type="Collect",
                keys=("coord", "grid_coord", "segment"),
                feat_keys=("normal",),  # <-- only normals
            ),
        ],
        test_mode=False,
    ),
    ...
    test=dict(
        ...
        test_cfg=dict(
            ...
            # aug_transform keeps only geometric transforms
            aug_transform=[
...
# ==== CHANGES FOR XYZ-ONLY (GEOMETRIC FEATURES) ====
