...
# ---------------------------------------------------------------------------
# NOGlobalBranch (Universal Branch)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# NOFusedPooling (With Universal Adapters)
# ---------------------------------------------------------------------------
...
class NOFusedPooling(PointModule):
    def __init__(
        ...
        if enable_no:
            if universal_no_branch is not None:
                # Use the single universal branch
                ...
                # Fallback to independent branches if universal is disabled
    ...
    def forward(self, point: Point) -> Point:
        if self.enable_no:
            # 1. Project layer channels to Universal channels (e.g. 256 -> 64)
            feat_down = self.down_proj(point.feat)
            
            # 2. Process via Universal Branch
            global_feat = self.no_branch(feat_down, point.grid_coord)
            
            # 3. Project Universal back to Target Out channels (e.g. 64 -> 256)
...
# ---------------------------------------------------------------------------
# NOFusedUnpooling (With Universal Adapters)
# ---------------------------------------------------------------------------
...
class PointTransformerV3_NO(PointModule):
    def __init__(
        ...
        fno_modes: int = 8,                 # Recommended 8 to save parameters
        ...
        universal_dim: int = 64,            # NEW: The fixed dimension of the Universal Branch
    ):
        super().__init__()
        ...
        # =========================================================
        # THE UNIVERSAL GLOBAL BRANCH
        # Instantiated exactly ONCE, used by all layers
        # =========================================================
        if share_no_branch:
        ...
        for s in range(self.num_stages):
            ...
            if s > 0:
                grid_size = (
                    ...
                    if adaptive_grid and self.universal_no_branch is None else base_grid_size
                )
            ...
            for i in range(enc_depths[s]):
                enc.add(
                    Block(
                        ...
                        mlp_ratio=mlp_ratio, drop_path=enc_drop_path_[i], norm_layer=ln_layer,
                        act_layer=act_layer, pre_norm=pre_norm, order_index=i % len(self.order),
                        cpe_indice_key=f"stage{s}", enable_flash=enable_flash,
            ...
            for i in range(dec_depths[s]):
                dec.add(
                    Block(
                        ...
                        mlp_ratio=mlp_ratio, drop_path=dec_drop_path_[i], norm_layer=ln_layer,
                        act_layer=act_layer, pre_norm=pre_norm, order_index=i % len(self.order),
                        cpe_indice_key=f"stage{s}", enable_flash=enable_flash,
                        upcast_attention=upcast_attention, upcast_softmax=upcast_softmax,
                    ),
