...
class NOGlobalBranch(PointModule):
    ...
    def _compute_flat_idx(self, point: Point, Gx: int, Gy: int, Gz: int) -> torch.Tensor:
        gc = point.grid_coord.long() - point.grid_coord.long().min(dim=0).values
        gc = gc.clamp(max=torch.tensor([Gx-1, Gy-1, Gz-1], device=gc.device))
        return gc[:, 0] * (Gy * Gz) + gc[:, 1] * Gz + gc[:, 2]
    ...
    def forward(self, point: Point) -> torch.Tensor:
        ...
        # ── cache flat_idx per stage to avoid recomputing ────────────────────
    ...
    def forward(self, point: Point) -> Point:
        ...
        if self.enable_no:
            ...
            if self.fusion == "add":
                ...
                alpha = self.gate(torch.cat([point.feat, feat_no_coarse], dim=-1))  # (N, C) in (0,1)
                point.feat = self.proj_concat(
                    alpha * feat_no_coarse + (1.0 - alpha) * point.feat
                )
            else:
