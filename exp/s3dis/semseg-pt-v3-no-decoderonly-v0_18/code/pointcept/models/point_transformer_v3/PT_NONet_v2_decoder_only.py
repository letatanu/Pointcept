...
class MaskedAveragePoolingBlock(nn.Module):
    def __init__(self, channels: int, num_queries: int = 128):
        super().__init__()
        self.mask_predictor = nn.Linear(channels, num_queries)
    ...
    def forward(self, feat: torch.Tensor) -> torch.Tensor:
        # feat: [N, C]
        mask_logits = self.mask_predictor(feat)
        
        # Softmax over the N dimension to create pooling weights that sum to 1 per query
        mask_probs = torch.nn.functional.softmax(mask_logits, dim=0) # [N, K]
        
        # Pool: [K, C]
        pooled_features = torch.einsum("nc,nk->kc", feat, mask_probs)
        
        # Process region-level context
        processed_features = self.region_mlp(pooled_features)
        
        # Broadcast back to points: [N, C]
        fused_back = torch.einsum("kc,nk->nc", processed_features, mask_probs)
        
        return feat + fused_back
...
class NOGlobalBranch(nn.Module):
    def __init__(
        ...
        NO_type: str = "WNO",
        num_queries: int = 128,  # New param for MAP
    ):
        super().__init__()
        ...
        if NO_type == "FNO":
            ...
            self.no = MaskedAveragePoolingBlock(channels, num_queries=num_queries)
    ...
    def forward(self, feat: torch.Tensor, point: Point) -> torch.Tensor:
        ...
        # --- NEW: MAP operates directly on point features ---
        if self.NO_type == "MAP":
            # Just pass the point features directly through the Masked Average Pooling block
            feat_out = self.no(feat)
            return self.norm(feat_out)
            
        # --- Standard Grid Projection for WNO / FNO / MLP ---
    ...
    def __init__(
        ...
        for idx, s in enumerate(range(self.num_stages - 2, -1, -1)):
            ...
            for i in range(self.dec_depths[idx]):
                ...
                stage_seq.add(base_block, name=f"block{i}")
