...
class OrderingLoss(nn.Module):
    ...
    def forward(self, scores, coords, batch_ids, offset, z_target=None):
        ...
        if z_target is not None:
            s = scores_1d.squeeze(1)                                          # (N,)
            if z_target.dim() == 1:
                # single order — backward-compatible
                best_target = z_target                                        # (N,)
            else:
                with torch.no_grad():
                    # Pick the single order whose mean MSE is lowest globally
                    mse_per_order = ((z_target - s.unsqueeze(0)) ** 2).mean(dim=1)  # (K,)
                    best_k = mse_per_order.argmin()           # scalar: one consistent order
                best_target = z_target[best_k]                # (N,)

            loss_z = F.mse_loss(s, best_target) * self.z_loss_weight
    ...
    def forward(self, data_dict):
        ...
        if self.training:
            ...
            point.serialization(
                ...
                shuffle_orders=not use_learned,   # Shuffle only before warmup
                compute_codes=True,
            )
        else:
            target_codes = None
