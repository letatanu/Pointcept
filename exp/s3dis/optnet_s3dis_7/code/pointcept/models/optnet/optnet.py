...
class OPTNet(PointTransformerV3):
    ...
    def compute_ordering_loss(self, point, scores):
        ...
        # --- 1. Subsample for SoftSort (Avoid OOM) ---
        # We pick 'num_chunks' random subsets of size 'chunk_size'
        chunk_size = 512 
        num_chunks = 4
        N = scores.shape[0]
        
        # Skip if point cloud is too small
        if N < chunk_size:
            return torch.tensor(0.0, device=scores.device)

        soft_sorter = SoftSort(tau=0.1) # Low tau = closer to hard sort

        for _ in range(num_chunks):
            # Randomly select indices
            rand_idx = torch.randperm(N, device=scores.device)[:chunk_size]
            
            # Extract scores and features for this chunk
            chunk_scores = scores[rand_idx].unsqueeze(0) # (1, 512, 1)
            chunk_feat = point.feat[rand_idx].unsqueeze(0) # (1, 512, C) (Use Color/Normal!)
            
            # Run SoftSort
            # This is differentiable! Gradients flow to 'chunk_scores' -> 'scores' -> 'sorter'
            sorted_chunk_feat = soft_sorter(chunk_scores, chunk_feat).squeeze(0) # (512, C)
            
            # --- 2. Feature Smoothness Loss ---
            # If sorted correctly, adjacent points should have similar features.
            # Minimize distance between f_i and f_{i+1}
            diffs = sorted_chunk_feat[1:] - sorted_chunk_feat[:-1]
            loss_smoothness = (diffs ** 2).mean()
            loss_list.append(loss_smoothness)

        # --- 3. Global Distribution Regularization ---
        # Keep scores spread out 0..1
        ...
        return sum(loss_list) / num_chunks + loss_dist
    # def compute_ordering_loss(self, point, scores):
    #     """
