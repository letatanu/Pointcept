...
class OPTNet(PointTransformerV3):
    ...
    def compute_ordering_loss(self, point, scores):
        ...
        num_chunks = 4
        N = scores.shape[0]
        num_orders = scores.shape[1] # Check how many orders we have (e.g., 4)
        
        # Skip if point cloud is too small
        if N < chunk_size:
            return torch.tensor(0.0, device=scores.device)

        soft_sorter = SoftSort(tau=0.001)

        for _ in range(num_chunks):
            # Randomly select indices
            rand_idx = torch.randperm(N, device=scores.device)[:chunk_size]
            
            # Extract features: (1, 512, C)
            chunk_feat = point.feat[rand_idx].unsqueeze(0) 
            
            # Loop over each learned order head (Z, Z-trans, etc.)
            for k in range(num_orders):
                # Extract Single Score Column: (N, 4) -> (N, 1) -> (1, 512, 1)
                # We MUST slice [:, k:k+1] to keep the last dimension as 1
                chunk_scores_k = scores[rand_idx, k].view(1, chunk_size, 1)
                
                # Run SoftSort on this specific order
                sorted_chunk_feat = soft_sorter(chunk_scores_k, chunk_feat).squeeze(0) # (512, C)
        ...
        # Enforce 0-1 distribution for EACH head independently
        loss_dist_list = []
        for k in range(num_orders):
        ...
        return (sum(loss_list) / len(loss_list)) * 100.0 + loss_dist * 10.0    # def compute_ordering_loss(self, point, scores):
    #     """
