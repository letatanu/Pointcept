...
class SelfSupervisedOrderingLoss(nn.Module):
    ...
    def _compute_contrastive_loss(self, scores, coords, offset):
        ...
        try:
            ...
            # else:
            #     idx_all = self._manual_knn(coords, offset, k_far)
            #     idx_near = idx_all[:, :k_near]
            #     idx_far = idx_all[:, k_near:]
    ...
    def _compute_smoothness_loss(self, scores, coords, offset):
        ...
        try:
            ...
            # else:
            #     idx = self._manual_knn(coords, offset, k_smooth)
    ...
    def _manual_knn(self, coords, offset, k):
        """Fallback manual k-NN computation per batch."""
        ...
        if offset.numel() == 0:
            ...
            offset_list = [0] + offset.tolist()
            ranges = [(offset_list[i], offset_list[i+1]) for i in range(len(offset_list)-1)]
        ...
        for start, end in ranges:
            ...
            L = coords_batch.shape[0]
            k_actual = min(k, L - 1)
            
            if k_actual < 1:
                idx_list.append(torch.zeros((L, max(k, 1)), device=device, dtype=torch.long) + start)
                continue
            
            dists = torch.cdist(coords_batch, coords_batch)
            _, indices = torch.topk(dists, k_actual + 1, dim=1, largest=False)
            indices = indices[:, 1:]
            
            if indices.shape[1] < k:
                pad_size = k - indices.shape[1]
                padding = indices[:, -1:].expand(-1, pad_size)
