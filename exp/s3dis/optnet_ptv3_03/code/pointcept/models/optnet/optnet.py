...
class OPTNetSegmentor(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        if self.training:
            ...
            # --- SAFETY FIX 2.0: Robust Sanitization ---
            # 1. Force Long (int64) to ensure -1 is handled correctly (not wrapped as 255)
            # 2. Clone to avoid side effects
            if not target.dtype == torch.int64:
                target = target.long()
            
            # 3. Create a mask of VALID labels [0, num_classes-1]
            #    Any label outside this range (e.g. -100, 255, etc.) becomes -1
            valid_mask = (target >= 0) & (target < self.num_classes)
            
            # Only clone/modify if necessary to save memory
