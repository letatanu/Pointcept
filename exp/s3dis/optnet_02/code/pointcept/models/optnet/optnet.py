"""
Scatter-OPTNet with Cluster Attention (Fixed).

- Fixed: ScatterOPTBlock now respects 'win_size' from config (was hardcoded 128).
- Fixed: Reduced distribution loss weight to prevent geometric distortion.
- Fixed: Sorter defaults to geometry-only to avoid color-based grouping artifacts.
"""
...
class PointSorter(nn.Module):
    ...
    Refined: 'use_feat' flag to control if features (color) influence sorting.
    """
    def __init__(self, in_channels, hidden_dim=64, use_feat=False):
        super().__init__()
        self.use_feat = use_feat
        # Input dim is 3 (coord) + in_channels (feat) if used
        input_dim = 3 + in_channels if use_feat else 3
    ...
    def forward(self, point):
        # By default, rely on geometry (coord) to preserve structural clusters.
        # Adding features (color) can sometimes cause beams (white) to merge with ceiling (white).
        if self.use_feat and point.feat is not None:
...
class ClusterAttention(nn.Module):
    """
    Cluster Attention with Soft Binning (Differentiable).
    """
    ...
    def forward(self, x, idx_floor, idx_ceil, alpha, total_buckets):
        """
        x: (N, C)
        idx_floor: (N,) floor bucket index
        idx_ceil: (N,) ceil bucket index
        alpha: (N, 1) interpolation weight
        total_buckets: int
        """
        N, C = x.shape
        ...
        # Floor
        k_sum.index_add_(0, idx_floor, k * w_floor)
        v_sum.index_add_(0, idx_floor, v * w_floor)
        counts.index_add_(0, idx_floor, w_floor)

        # Ceil
        k_sum.index_add_(0, idx_ceil, k * w_ceil)
        v_sum.index_add_(0, idx_ceil, v * w_ceil)
        counts.index_add_(0, idx_ceil, w_ceil)

        # Mean
        counts = counts.clamp(min=1e-5)
        k_centroids = k_sum / counts
        v_centroids = v_sum / counts

        # 3. Soft Gather
        k_floor = k_centroids[idx_floor]
        k_ceil = k_centroids[idx_ceil]
        k_context = k_floor * w_floor + k_ceil * w_ceil
        
        v_floor = v_centroids[idx_floor]
        v_ceil = v_centroids[idx_ceil]
        v_context = v_floor * w_floor + v_ceil * w_ceil
...
class ScatterOPTBlock(nn.Module):
    """
    Scatter-Gather Block with Soft Permutation.
    """
    def __init__(self, channels, num_heads=4, win_size=32, dropout=0.0, ffn_ratio=2.0, indice_key=None, norm_layer=nn.BatchNorm1d, enable_spcpe=False):
        super().__init__()
        self.win_size = win_size  # Fixed: store win_size
    ...
    def forward(self, point, lengths, sorter_scores):
        ...
        # 1. Soft Bucketing
        # Fixed: use self.win_size instead of hardcoded 128
        n_buckets_per_batch = (lengths.float() / self.win_size).ceil().long()
        
        batch_ids = point.batch
        num_buckets_point = n_buckets_per_batch[batch_ids]
        batch_offset = torch.cat([torch.tensor([0], device=x.device), torch.cumsum(n_buckets_per_batch, dim=0)[:-1]])[batch_ids]
        total_buckets = n_buckets_per_batch.sum().item()

        # Continuous index
        float_idx = sorter_scores * (num_buckets_point.float() - 1.0)
        idx_floor = float_idx.floor().long()
        idx_ceil = (idx_floor + 1).clamp(max=num_buckets_point - 1)
        alpha = (float_idx - idx_floor.float()).unsqueeze(-1)

        global_floor = idx_floor + batch_offset
        global_ceil = idx_ceil + batch_offset

        # 2. Soft Attention
        x_out = self.attn(x, global_floor, global_ceil, alpha, total_buckets)
...
class OPTNet(PointModule):
    def __init__(self, in_channels=6, embed_dim=128, enc_depths=(2, 2, 6, 2), dec_depths=(1, 1, 1, 1),
                 ...
                 dropout=0.0, ffn_ratio=3.0, win_chunk=256, ordering_loss_weight=0.5, ordering_k=16, warmup_epoch=1,
        ...
        # Learnable Sorter (ICPR-style)
        # Fixed: use_feat=False to rely on geometry primarily.
        self.sorter = PointSorter(embed_dim, hidden_dim=64, use_feat=False)
        ...
        for si in range(self.num_stages):
            self.enc_blocks.append(nn.ModuleList([
                ScatterOPTBlock(curr_dim, num_heads=num_heads, win_size=win_sizes[si], dropout=dropout, ffn_ratio=ffn_ratio,
    ...
    def compute_ordering_loss(self, point, scores):
        """
        ICPR-style self-supervised ordering loss:
        1) Locality: KNN neighbors should have similar scores
        2) Distribution: scores should cover [0,1] to avoid collapse
        """
        scores = scores.view(-1)
        
        # Ensure we have an offset
        ...
        # Fixed: Lower weight for distribution loss (0.1) to avoid breaking geometric clusters too aggressively
        loss_dist = ((sorted_scores - target) ** 2).mean() * 0.1
    ...
    def forward(self, data_dict):
        ...
        # 1. Predict Scores (ICPR-style sorter reads point.{coord,feat})
        point.feat = x
        ...
        # 1a. Self-supervised ordering loss (no teacher matching)
        ...
        # 1b. (Optional) keep warmup bucketing using intrinsic scores, but do NOT MSE-train on them
        if self.training and current_epoch < self.warmup_epoch:
            strategy = random.choice(self.strategies)
            with torch.no_grad():
                bucket_scores = compute_intrinsic_ordering_scores(point.coord, point.batch, strategy=strategy)
        else:
