...
class PointTransformerV3_NO_DecoderOnly(PointModule):
    def __init__(
        ...
        for idx, s in enumerate(range(self.num_stages - 2, -1, -1)):
            ...
            stage_seq = PointSequential()
            # stronger coord_drop at finer (larger idx) stages
            c_drop = 0.4 if idx >= (self.num_stages - 3) else 0.0
            for i in range(self.dec_depths[idx]):
                ...
                stage_seq.add(
                    QuatRPEBlockWrapper(base_block, enc_channels[s], coord_drop=c_drop),
                    name=f"block{i}",
                )
