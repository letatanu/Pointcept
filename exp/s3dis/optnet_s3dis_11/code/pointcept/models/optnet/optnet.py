...
class OPTNet(PointTransformerV3):
    ...
    def compute_ordering_loss(self, point, scores):
        ...
        for _ in range(num_chunks):
            ...
            # 3. Create One-Hot Labels (The Ground Truth Signal)
            # We want to sort these so they form clean blocks.
            labels = point.segment[rand_idx].long()
            
            # Handle ignore_index (-1) by setting them to 0 or a dummy class
            # (In S3DIS -1 is rare in training crops, but good to be safe)
            labels[labels == -1] = 0 
            
            chunk_one_hot = torch.nn.functional.one_hot(labels, num_classes).float() # (1, 1024, 13)
            chunk_one_hot = chunk_one_hot.unsqueeze(0)

            for k in range(num_orders):
                chunk_scores_k = scores[rand_idx, k].view(1, chunk_size, 1)
                
                # 4. Soft Sort the LABELS
                # If sorted correctly: [Floor, Floor, Wall, Wall] -> OneHot transitions are rare.
                # If sorted badly: [Floor, Wall, Floor, Wall] -> OneHot transitions are frequent.
                sorted_labels = soft_sorter(chunk_scores_k, chunk_one_hot).squeeze(0)
                
                # 5. Minimize Label Flickering (Total Variation Loss)
                # We simply want adjacent points in the sorted list to have the SAME label.
                diffs = sorted_labels[1:] - sorted_labels[:-1]
                loss_cls_grouping = diffs.abs().mean()
                
                loss_list.append(loss_cls_grouping)

        # 6. Distribution Regularization (Keep this to prevent collapse)
        loss_dist_list = []
        for k in range(num_orders):
        ...
        # 7. Final Loss
        # High weight on grouping because max possible loss is small (0~2)
        total_loss = (sum(loss_list) / len(loss_list)) * 10.0 + loss_dist * 10.0
