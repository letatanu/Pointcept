...
class OrderingLoss(nn.Module):
    def __init__(self, ordering_k=16, z_loss_weight=10.0):
        super().__init__()
        self.ordering_k    = ordering_k
        self.z_loss_weight = z_loss_weight

    def forward(self, scores, coords, batch_ids, offset, z_target=None):
        scores_1d = scores.view(-1, 1)
        # scores_1d = torch.nan_to_num(scores_1d, nan=0.5)
        ...
        loss_locality     = self._locality_loss(scores_1d, coords, offset)
        loss_distribution = self._distribution_loss(scores_1d, batch_ids)

        total = loss_locality + loss_distribution
        loss_dict = {
        ...
        if z_target is not None:
            loss_z = F.mse_loss(scores_1d, z_target.view(-1, 1)) * self.z_loss_weight
    ...
    def _locality_loss(self, scores, coords, offset):
        ...
        return F.mse_loss(
            scores.unsqueeze(1).expand_as(neighbor_scores),
            neighbor_scores
        )
...
class PointSorter(nn.Module):
    ...
    def __init__(
        ...
        ordering_k=16,
        z_loss_weight=3.0,  # Lowered weight to prevent collapse
        num_classes=13,
...
class OPTNet(PointTransformerV3):
    ...
    def forward(self, data_dict):
        ...
        if self.training:
            ...
            # SAVE THE RAW TARGET CODES HERE before they are overwritten!
            target_codes = point.serialized_code.float().clone()
        else:
            target_codes = None
        ...
        # ------------------------------------------------------------------
        segment        = data_dict.get("segment", None) if self.training else None
        learned_scores = self.sorter(point, segment=segment)
        ...
        if self.training and self.ordering_loss_weight > 0:
            # Normalize each curve's codes to [0, 1] across the batch
            # target_codes shape: [num_orders, N]
            ...
            loss_ord, loss_dict = self.sorter.compute_loss(
                ...
                point.batch,
                point.offset,
                z_target=z_targets, # Pass multi-targets
            )
