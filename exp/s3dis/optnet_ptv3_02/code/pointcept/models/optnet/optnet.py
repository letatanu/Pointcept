...
class PointSorter(nn.Module):
    ...
    def forward(self, point):
        ...
        inp = torch.clamp(inp, -100.0, 100.0)
        ...
        for i in range(scores_t.shape[0]):
            # Use stable sort to be safe
            order = torch.argsort(scores_t[i].detach(), stable=True)
...
class OPTNet(PointTransformerV3):
    ...
    def forward(self, data_dict):
        ...
        if not need_standard_z:
            ...
            # Use Rank (inverse) for uniform pooling
            # Unsqueeze to [1, N] for shape compatibility
