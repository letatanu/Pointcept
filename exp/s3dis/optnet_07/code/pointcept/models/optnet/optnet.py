...
class PointSorter(nn.Module):
    def __init__(self, in_channels, hidden_dim=64):
        ...
        self._init_weights_zorder()
    
    def _init_weights_zorder(self):
        """
        Initialize weights to approximate Z-order (Morton code) curve.
        Z-order interleaves bits of x, y, z coordinates to create a space-filling curve.
        We initialize the first layer to emphasize coordinate importance.
        """
        # Zero out feature weights initially
        nn.init.constant_(self.mlp[0].weight[:, 3:], 0.0)
        
        # Initialize coordinate weights for Z-order approximation
        # Z-order priority: interleave x, y, z bits
        # We use different scales to approximate the bit interleaving
        with torch.no_grad():
            hidden_dim = self.mlp[0].weight.shape[0]
            
            # Distribute neurons to different coordinate combinations
            # Group 1: emphasize X (highest bits)
            n_per_coord = hidden_dim // 3
            self.mlp[0].weight[:n_per_coord, 0] = 1.0  # x
            self.mlp[0].weight[:n_per_coord, 1] = 0.5  # y
            self.mlp[0].weight[:n_per_coord, 2] = 0.25 # z
            
            # Group 2: emphasize Y
            self.mlp[0].weight[n_per_coord:2*n_per_coord, 0] = 0.5  # x
            self.mlp[0].weight[n_per_coord:2*n_per_coord, 1] = 1.0  # y
            self.mlp[0].weight[n_per_coord:2*n_per_coord, 2] = 0.5  # z
            
            # Group 3: emphasize Z (lowest bits)
            self.mlp[0].weight[2*n_per_coord:, 0] = 0.25 # x
            self.mlp[0].weight[2*n_per_coord:, 1] = 0.5  # y
            self.mlp[0].weight[2*n_per_coord:, 2] = 1.0  # z
            
            # Add small random noise for diversity
            self.mlp[0].weight[:, :3] += torch.randn_like(self.mlp[0].weight[:, :3]) * 0.01
        
        # Initialize output bias to center range
        nn.init.constant_(self.mlp[6].bias, 0.0)
        
        # Initialize intermediate layers with small weights for smooth start
...
class OPTNet(PointModule):
    ...
    def compute_ordering_loss(self, point: Point, scores: torch.Tensor):
        ...
        # KNN locality loss (contrastive: pull neighbors together)
        idx = pointops.knn_query(self.ordering_k, point.coord, offset)[0]
        neighbor_scores = scores[idx.long()]
        loss_locality = ((scores.unsqueeze(1) - neighbor_scores) ** 2).mean()
        
        # Contrastive: push random pairs apart with margin
        N = scores.size(0)
        num_neg_pairs = min(N, 2048)
        idx1 = torch.randint(0, N, (num_neg_pairs,), device=scores.device)
        idx2 = torch.randint(0, N, (num_neg_pairs,), device=scores.device)
        neg_dist = (scores[idx1] - scores[idx2]) ** 2
        loss_contrast = F.relu(0.5 - neg_dist).mean()
        
        # Optional: Keep distribution regularization
        ...
        return loss_locality + loss_contrast + 0.1 * loss_dist

    # def compute_ordering_loss(self, point: Point, scores: torch.Tensor):
    #     """
    #     Contrastive loss for self-supervised point ordering
    #     Pull spatially close neighbors together, push distant points apart
    #     """
    #     scores = scores.view(-1)
    #     coords = point.coord
    #     offset = point.offset if (hasattr(point, "offset") and point.offset is not None) else torch.tensor([scores.numel()], device=scores.device)
        
    #     # Get k-nearest neighbors as positive pairs
    #     neighbor_idx = pointops.knn_query(self.ordering_k, coords, offset)[0]  # [N, k]
    #     neighbor_scores = scores[neighbor_idx.long()]  # [N, k]
        
    #     # Positive loss: minimize distance between anchor and neighbors
    #     anchor_scores = scores.unsqueeze(1).expand_as(neighbor_scores)
    #     pos_loss = ((anchor_scores - neighbor_scores) ** 2).mean()
        
    #     # Negative loss: maximize distance between random pairs with margin
    #     N = scores.size(0)
    #     num_neg_pairs = min(N * 2, 4096)  # Sample subset for efficiency
        
    #     # Sample random negatives (should be from same batch to be meaningful)
    ...
    #         # Sample random pairs within this batch
    #         n_pairs = min(batch_size * 2, 2048)
    #         idx1 = torch.randint(0, batch_size, (n_pairs,), device=scores.device)
    #         idx2 = torch.randint(0, batch_size, (n_pairs,), device=scores.device)
            
    #         # Compute distances
    #         neg_dist = (batch_scores[idx1] - batch_scores[idx2]) ** 2
            
    #         # Apply margin: push apart if distance < margin
    #         neg_loss_list.append(F.relu(0.5 - neg_dist).mean())
        
    #     neg_loss = torch.stack(neg_loss_list).mean() if neg_loss_list else torch.tensor(0.0, device=scores.device)
        
    #     return pos_loss + neg_loss

    # def compute_ordering_loss(self, point: Point, scores: torch.Tensor):
    #     """
    #     InfoNCE contrastive loss for point ordering
    #     """
    #     scores = scores.view(-1)
    #     N = scores.size(0)
    #     coords = point.coord
    #     offset = point.offset if (hasattr(point, "offset") and point.offset is not None) else torch.tensor([N], device=scores.device)
    #     batch = point.batch.long()
        
    #     # Get k-nearest neighbors as positive pairs
    #     neighbor_idx = pointops.knn_query(self.ordering_k, coords, offset)[0]  # [N, k]
        
    #     # Normalize scores to unit sphere for cosine similarity
    #     score_norm = F.normalize(scores.unsqueeze(-1), dim=0)  # [N, 1]
        
    #     temperature = 0.07
    #     loss_list = []
        
    #     # Process per batch to avoid cross-batch contamination
    #     for b in batch.unique():
    #         mask = (batch == b)
    #         batch_indices = torch.where(mask)[0]
    #         batch_size = batch_indices.size(0)
            
    #         if batch_size < self.ordering_k + 2:
    #             continue
            
    #         # Sample anchors (use subset for efficiency)
    #         num_anchors = min(batch_size, 512)
    #         anchor_sample_idx = torch.randperm(batch_size, device=scores.device)[:num_anchors]
    #         anchor_global_idx = batch_indices[anchor_sample_idx]
            
    #         for anchor_idx in anchor_global_idx:
    #             # Positive samples: k-nearest neighbors
    #             pos_indices = neighbor_idx[anchor_idx].long()
                
    #             # Negative samples: random points from same batch (not neighbors)
    #             neg_sample_size = min(256, batch_size - self.ordering_k)
    #             neg_candidates = batch_indices[torch.randperm(batch_size, device=scores.device)]
                
    #             # Filter out the anchor and its neighbors
    #             neighbor_set = set(pos_indices.tolist() + [anchor_idx.item()])
    #             neg_indices = torch.tensor(
    #                 [idx.item() for idx in neg_candidates if idx.item() not in neighbor_set][:neg_sample_size],
    ...
    #             anchor_embed = score_norm[anchor_idx]  # [1]
    #             pos_embeds = score_norm[pos_indices]    # [k, 1]
    #             neg_embeds = score_norm[neg_indices]    # [num_neg, 1]
                
    #             # Positive similarities
    #             pos_sim = (anchor_embed * pos_embeds).sum(dim=-1) / temperature  # [k]
                
    #             # Negative similarities
    #             neg_sim = (anchor_embed * neg_embeds).sum(dim=-1) / temperature  # [num_neg]
                
    #             # InfoNCE loss: -log(exp(pos) / (exp(pos) + sum(exp(neg))))
    #             logits = torch.cat([pos_sim, neg_sim])  # [k + num_neg]
    #             labels = torch.zeros(self.ordering_k, dtype=torch.long, device=scores.device)
                
    #             # Use cross-entropy for multi-positive case
    #             loss = F.cross_entropy(
    #                 logits.unsqueeze(0).repeat(self.ordering_k, 1), 
    #                 labels
    #             )
