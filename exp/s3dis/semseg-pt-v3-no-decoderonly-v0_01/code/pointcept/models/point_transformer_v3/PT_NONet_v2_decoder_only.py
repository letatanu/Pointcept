...
class PointTransformerV3_NO_DecoderOnly(PointModule):
    def __init__(
        ...
        NO_type: str = "WNO",
        pool_reduce: str = "max",
        # FIX 1: add missing params that config passes
        head_out_channels: int = 64,
        head_fusion: str = "sum",   # kept for config compat, unused
    ):
        super().__init__()
        ...
        # --- Scaffold: pure GridPooling down (stores pooling_inverse at each level) ---
        ...
        for i in range(enc_depths[s]):
            ...
            self.coarse_blocks.add(QuatRPEBlockWrapper(base_block, enc_channels[s]), name=f"block{i}")

        # --- Decoder stages (coarse -> fine) ---
        # FIX 2: store unpool modules and block lists separately to avoid
        # passing fine_point through PointSequential
        ...
        for s in range(self.num_stages - 2, -1, -1):
            ...
            for i in range(enc_depths[s]):
                ...
                stage_seq.add(QuatRPEBlockWrapper(base_block, enc_channels[s]), name=f"block{i}")
            self.dec_blocks.append(stage_seq)

        # FIX 1 cont: output projection enc_channels[0] -> head_out_channels
    ...
    def forward(self, data_dict: dict) -> Point:
        ...
        # 2. Scaffold: pool down, storing pooling_inverse at each level
        scaffold = [point]
        for pool in self.pre_pool_stages:
            point = pool(point)
            scaffold.append(point)
        # scaffold[0]=finest ... scaffold[-1]=coarsest

        # 3. Global scene understanding at coarsest scale
        point = self.coarse_blocks(scaffold[-1])

        # 4. Decode: coarse -> fine
        # FIX 2: call unpool directly (needs two Point args), then blocks normally
        for i, (unpool, blocks) in enumerate(zip(self.dec_unpool, self.dec_blocks)):
            fine_point = scaffold[-(i + 2)]
            point      = unpool(point, fine_point)   # NOFusedGridUnpooling
            point      = blocks(point)               # PointSequential of QuatRPEBlockWrapper+Block

        # 5. Final projection to head_out_channels
        point.feat = self.output_proj(point.feat)
        point.sparse_conv_feat = point.sparse_conv_feat.replace_feature(point.feat)

        # FIX 3: clear pooling chain so DefaultSegmentorV3 doesn't re-walk it
