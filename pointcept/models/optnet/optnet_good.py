...
class ClusterAttention(nn.Module):
    """
    Explicit Attention Block for ScatterOPT.
    Performs Point-to-Centroid Attention with Dual Aggregation (Mean + Max).
    """
    def __init__(self, channels, num_heads=4, qkv_bias=True, attn_drop=0.0, proj_drop=0.0):
        super().__init__()
        self.num_heads = num_heads
        ...
        # Gating now accepts (Mean + Max) context, so input dim is channels * 2
        self.attn_gate = nn.Sequential(
        ...
        # Project Max+Mean V context back to channels
    ...
    def forward(self, x, cluster_ids, total_buckets):
        ...
        # 2. Aggregate K and V (Mean + Max)
        # Initialize buffers
        k_sum = torch.zeros((total_buckets, C), device=x.device, dtype=x.dtype)
        v_sum = torch.zeros((total_buckets, C), device=x.device, dtype=x.dtype)
        
        # Init Max buffers with very small number
        k_max = torch.full((total_buckets, C), -1e9, device=x.device, dtype=x.dtype)
        v_max = torch.full((total_buckets, C), -1e9, device=x.device, dtype=x.dtype)
        ...
        # If older, use scatter_max from torch_scatter if available, or just stick to Mean if necessary.
        # Assuming modern PyTorch as per provided files using scatter_reduce elsewhere.
        k_max = k_max.scatter_reduce(0, cluster_ids.unsqueeze(1).expand(-1, C), k, reduce="amax", include_self=False)
        v_max = v_max.scatter_reduce(0, cluster_ids.unsqueeze(1).expand(-1, C), v, reduce="amax", include_self=False)
        
        # Handle empty buckets for max (if any remain -1e9, set to 0)
        ...
        # We need to match dimensions for the interaction.
        # Q is (N, C), K_context is (N, 2*C). 
        # We allow Q to attend to both Mean and Max statistics.
        
        # Expand Q to match K_context for interaction
        ...
        # 5. Output
        # Combine V context features first
